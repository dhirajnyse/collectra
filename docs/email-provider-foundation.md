# Email Provider Foundation

## Current Version

`v1.3.0 - Email provider foundation`

## Goal

Let Collectra send explicitly approved queued email messages without exposing provider secrets in browser code.

## Why This Exists

Collections messages are high-trust customer communications. Collectra should keep the safe path:

1. AI drafts a follow-up.
2. User reviews the draft.
3. User queues the approved message.
4. User confirms the send action.
5. Server-side code calls the email provider and writes an audit event.

## Data Model

`workspace_email_settings` stores:

- `workspace_id`
- `created_by`
- `provider`
- `from_name`
- `from_email`
- `reply_to`
- `status`
- `metadata`

The first supported provider is `resend`.

## Edge Function

`supabase/functions/send-queued-email`:

- Validates the signed-in Supabase user.
- Confirms the user is a member of the workspace.
- Loads a queued email from `outbound_messages`.
- Loads active workspace email settings.
- Calls Resend from the server.
- Updates the queue to `sent` or `failed`.
- Writes `outbound_message.sent` or `outbound_message.failed` to `audit_logs`.

## Required Secrets

Set these in Supabase, not in `.env.local`:

```powershell
supabase secrets set EMAIL_PROVIDER=resend
supabase secrets set RESEND_API_KEY=your-resend-key
```

## Deploy Function

```powershell
cd C:\Users\dhiraj\Documents\Codex\Collectra\platform
supabase functions deploy send-queued-email
```

## Local Test Path

1. Run `platform/supabase/schema.sql`.
2. Start the platform app.
3. Sign in with magic link.
4. Create or load a workspace.
5. Generate an AI draft from an open invoice.
6. Queue the approved draft for email.
7. Save active sender settings in **Email provider**.
8. Click **Send email** in outbound review.
9. Confirm the message status changes from `queued` to `sent` or `failed`.
10. Confirm the audit trail records the send result.

## Guardrails

- Provider keys stay in Supabase Edge Function secrets.
- Browser code never receives the Resend API key.
- Email sends require an existing queued message.
- Email sends require active workspace sender settings.
- The Edge Function validates workspace membership before sending.
- Failed provider responses update the queue and audit trail.
- Production sender domains should be verified before sending customer emails.
